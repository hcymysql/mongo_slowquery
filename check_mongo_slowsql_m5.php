...
-----------------------------------
MongoDB 5.0:
db.getSiblingDB("samples").system.profile.find({millis:{$gte:1000}},{millis:1,ns:1,command:1,ts:1,client:1,user:1}).sort({ts:-1}).limit(1000)
*/
	$where=array("millis" => array('$gte' => (int)$threshold_slow_ms ));
	$fields=array("millis" => 1,"ns" => 1,"command" => 1,"ts" => 1, "client" => 1, "user" => 1);
...
function init(){
	...
	foreach ($cursor as $doc) {
		$find = json_encode($doc['command']['find']);
		$filter = json_encode($doc['command']['filter']);
		$limit = json_encode($doc['command']['limit']);
		...
		$querysql .= "\"find\" : " . $find . ", \"filter\" : " . $filter . ", \"limit\" : " . $limit . PHP_EOL;
		$querysql = "{" . $querysql . "}"; // 添加大括号以构成合法的JSON对象
		$querysql = json_encode(json_decode($querysql));
		//print_r($doc); //打开调试
...
function incr(){
	...
	foreach ($cursor as $doc) {
		//$querysql = (json_encode($doc['command']))?json_encode($doc['command']):json_encode($doc['aggregate']);
		$find = json_encode($doc['command']['find']);
		$filter = json_encode($doc['command']['filter']);
		$limit = json_encode($doc['command']['limit']);
		$exec_time = round($doc['millis']/1000,2); //单位转换为秒
